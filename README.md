...
| `WEB_CONCURRENCY` | `1` | Number of uvicorn workers. With the in-memory job store (`EMAIL_VERIFIER_JOBS_BACKEND=memory` or `auto` with no Firebase), uploads to one worker are invisible to siblings — so the systemd unit pins `--workers 1` and the app logs a loud warning at startup if you set this higher. With `EMAIL_VERIFIER_JOBS_BACKEND=firestore` jobs are persisted to Firestore and `>1` is safe; the warning auto-suppresses. |
| `EMAIL_VERIFIER_JOBS_BACKEND` | `auto` | Where bulk-job state lives: `auto` (Firestore if Firebase Admin is configured, else in-memory), `memory` (always in-memory; jobs vanish on restart), or `firestore` (always Firestore; raises at startup if `FIREBASE_ADMIN_CREDENTIALS` is missing). Use `firestore` on multi-worker deploys so a misconfiguration can't silently regress to in-memory and lose cross-worker visibility. |
